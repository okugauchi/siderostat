...
v0.3.2 の公式提供物はソースコードであり、事前ビルド済みの `.app`、`.pkg`、DMG、
...
これらの artifact と Apple Developer ID、notary profile、secure timestamp は、v0.3.2 の
ソースリリース受入条件ではない。
...
v0.3.2 のリリース artifact にはならない。

```sh
cargo xtask dmg-dev \
  --app-dir build/app-dev \
  --package dist/Siderostat-0.3.2.pkg \
  --version 0.3.2 \
  --build-number 11 \
  --output-dir dist \
  --verify
...
保持する。v0.3.2 のソース導入では `cargo xtask uninstall` を使用する。Uninstaller.app を含む DMG の導線は、
