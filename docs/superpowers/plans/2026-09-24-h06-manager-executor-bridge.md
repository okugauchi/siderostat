...
Redact backend errors before writing them to the journal by removing URL userinfo, bearer/token query values, and `raw build log` labels. Keep the original error private to tracing only after redaction.
