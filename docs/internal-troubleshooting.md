...
この文書は、`docs/spec.md`と実装済みのadmin API / CLI / logging / metricsに基づき、failure symptom別に該当手順を定める。運用の基本は [`docs/internal-operations.md`](internal-operations.md) を参照する。実DS4/modelを使う検証は [`docs/internal-installation.md`](internal-installation.md) に従う。

## 0. Install／first launch／Login Items

- 通常の導入は DMG 内の `.pkg` だけを Finder から実行する。`cargo xtask install` と手動 LaunchAgent
  登録は開発・診断用で、配布版と併用しない。
- install 成功後に `/Applications/Siderostat.app` が起動しない場合は、ログイン中の GUI session が
  存在するか、`/Applications/Siderostat.app` が存在するか、`/healthz` が応答するかを確認する。
- `Login Items` の承認要求は package の管理者認証とは別である。System Settings > General > Login Items
  で Siderostat と runtime background item の状態を確認し、必要なら承認してから Siderostat を再起動する。
- 通知が表示されない場合も cluster lifecycle を変更しない。まず `/cluster`、`/readyz`、LaunchAgent／
  Service Management の状態を read-only で採取する。

Uninstaller のエラー表示は、ユーザーデータ削除の失敗を意味しない。表示された対象 process／Trash／
receipt の状態を解消し、同じ `Siderostat Uninstaller.app` を再実行する。ユーザーデータを手動削除しない。
...
- 実 DS4 の source commit と native binary digest は、歴史的 baseline ではなく
  `docs/compatibility/ds4-b030961.md` の approved native binary 集合と Model/profile matrix に一致することを確認する。
- Mac 間 TP、layer-parallel RDMA、distributed DSpark は v0.3.0 の対応外である。未確認の DS4 option を
  `extra_args` に追加して復旧を試みない。
