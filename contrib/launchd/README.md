...
この文書は bundle 外の開発 binary を検証するための legacy 手順です。v0.3.4 の公式導入は
`Siderostat.app` を payload とする `.pkg` を macOS Installer で導入する方法です。利用者向けの
導入にこの文書の手順や `cargo xtask install --start` を使用しないでください。
